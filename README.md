How to check bytecode of a program
node --print-bytecode <path/filename.js>

## Chapters

- **chapter_01_basics**: Basic JavaScript and Playwright examples
- **chapter_02_Javascript_Concepts**: JavaScript concepts (hoisting, null vs undefined, etc.)
- **chapter_03_Literals**: Examples of all JavaScript literals in `literal.js` (numeric, string, boolean, null, undefined, object, array, RegExp, BigInt, symbol, function, template, nested). Also includes `Backtick_single_double.js` for a quick comparison of string quote types.
- **chapter_04_Operators**: JavaScript operators, including `Null_Nullish_OP.js` for nullish coalescing operator examples.
